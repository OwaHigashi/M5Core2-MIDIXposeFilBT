...
- **画面下部のバーをタッチでマスターボリューム調整** (左端=0 / 右端=最大)。本体に物理ボリュームが無いため、GM ユニバーサル Master Volume SysEx (`F0 7F 7F 04 01 00 vol F7`) で外部音源全体の音量を変えます (曲ごとの CC7 には触れません)。
- 入室は `C` 長押しによるグループ巡回経由 (`MIDI 管理 -> SMF プレーヤー`)。
- `C` は短押し・長押しともに**モード切り替え** (`SMF プレーヤー -> 転調`) 専用です。
...
- `C` 短押し・長押し: `SMF プレーヤー -> 転調` (再生中は停止し All Notes Off)
...
- `B`: モード別アクション (SMF: 曲送り・長押しで即選曲画面 / FILTER: Type 送り / 他)
- `C` 短押し: 現在グループ内の次モードへ (SMF 中は `転調` へ戻る)
- `C` 長押し: グループ巡回
- SMF プレーヤーの再生 / 停止は画面右上の ▶ / ■ タッチ、音量は画面下部のバー タッチ
...
3. `A` (曲戻り) / `B` (曲送り) で曲を選び、画面右上 ▶ で再生 / ■ で停止。`A` か `B` の長押しで「便利な即選曲」画面 (フォルダブラウザ) を開いてサブフォルダから選曲。音量は画面下部のバーをタッチ。
4. 別モードへ移るには `C` (短押し・長押しどちらでも、`SMF プレーヤー -> 転調`)。
...
SMF プレーヤー画面では、再生 / 停止と選曲 (即選曲画面)・音量はタッチ、曲送り / 戻りと選曲画面の開閉は `A` / `B` ボタン、モード切り替えは `C` ボタンで操作します。
